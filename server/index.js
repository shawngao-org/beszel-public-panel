...
app.get('/api/systems', async (req, res) => {
  try {
    ...
    const systems = await Promise.all(records.map(async (system) => {
      ...
      let details = {};
      try {
        try { details = await pb.collection('system_details').getFirstListItem(`system = "${system.id}"`); } catch (e) {}
        const historyRecords = await pb.collection('system_stats').getList(1, 20, { filter: `system = "${system.id}"`, sort: '-created' });
        recentHistory = historyRecords.items.map(h => ({ cpu: h.stats?.cpu || 0, mem: h.stats?.mp || 0, temp: sanitizeTemperature(h.stats?.t || h.stats?.temp) })).reverse();
        if ((!stats || Object.keys(stats).length === 0) && historyRecords.items.length > 0) { stats = historyRecords.items[0].stats; }
      } catch (e) {}
      if (stats) {
        const temperature = sanitizeTemperature(system.info?.sv) || sanitizeTemperature(stats.t) || sanitizeTemperature(stats.temp);
        return {
          ...system,
          details: { ...details, os_name: details.os_name || details.os || system.info?.os || 'Linux', mem_total: (stats.m || details.mem_total || 0) * 1024, disk_total: (stats.d || details.disk_total || 0) * 1024 },
          history: recentHistory,
          stats: { ...stats, cpu: stats.cpu || 0, mem: stats.mp || 0, disk: stats.dp || 0, temp: temperature, load: stats.la || (system.info?.la) || [0, 0, 0], net_in: Array.isArray(stats.b) ? stats.b[0] : 0, net_out: Array.isArray(stats.b) ? stats.b[1] : 0 }
        };
      }
  ...
  try {
    ...
    const history = systemRecords.items.map(record => {
      ...
      const item = {
        ...
        cpu: stats.cpu || 0, mem: stats.mp || 0, net_in: Array.isArray(stats.b) ? stats.b[0] : 0, net_out: Array.isArray(stats.b) ? stats.b[1] : 0,
        load_1m: Array.isArray(stats.la) ? stats.la[0] : 0, load_5m: Array.isArray(stats.la) ? stats.la[1] : 0, load_15m: Array.isArray(stats.la) ? stats.la[2] : 0,
        temp: sanitizeTemperature(stats.t || stats.temp), disk_p: stats.dp || 0, disk_read: Array.isArray(stats.dio) ? stats.dio[0] : 0, disk_write: Array.isArray(stats.dio) ? stats.dio[1] : 0,
        docker_cpu: parseFloat(docker.cpu.toFixed(2)), docker_net_in: docker.net_in, docker_net_out: docker.net_out
      };
      const rawTemp = stats.t || stats.temp || {};
      if (typeof rawTemp === 'object' && rawTemp !== null) { Object.entries(rawTemp).forEach(([key, val]) => { if (typeof val === 'number') item[`sensor_${key}`] = val; }); }
      return item;
    });
...
app.get(/.*/, (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    // Inject runtime config into window.__CONFIG__
    const runtimeConfig = {
      VITE_API_URL: process.env.VITE_API_URL || '',
    };
    const configScript = `<script>window.__CONFIG__ = ${JSON.stringify(runtimeConfig)};</script>`;
    // Inject before </head> to ensure it's available for app JS
